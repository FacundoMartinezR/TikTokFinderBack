...
async function getAccessToken() {
    ...
    const res = await (0, node_fetch_1.default)(`${PAYPAL_API}/v1/oauth2/token`, {
        method: "POST",
        headers: {
...
function extractUserIdFromReq(req) {
    try {
        ...
        console.log("No se pudo extraer userId del token en cookie (ok si se envía en el body):", e.message);
    }
    try {
        const authHeader = req.headers.authorization;
        if (authHeader && authHeader.startsWith("Bearer ")) {
            const bearer = authHeader.split(" ")[1];
            const payload = (0, jwt_1.verifyToken)(bearer);
            if (payload?.id)
                return payload.id;
        }
    }
    catch (e) {
        console.log("No se pudo extraer userId del Authorization header:", e.message);
    }
    // fallback a body
...
router.post("/create-subscription", async (req, res) => {
    try {
        ...
        const response = await (0, node_fetch_1.default)(`${PAYPAL_API}/v1/billing/subscriptions`, {
            method: "POST",
            headers: {
...
router.post("/check-subscription", async (req, res) => {
    try {
        ...
        const response = await (0, node_fetch_1.default)(`${PAYPAL_API}/v1/billing/subscriptions/${subscriptionId}`, {
        ...
        if (data.status === "ACTIVE") {
            ...
            if (userId) {
                await prisma_1.prisma.user.update({ where: { id: userId }, data: { role: "PAID" } });
