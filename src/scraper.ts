...
// ---------- seed writer ----------
const SEED_PATH = path.join(process.cwd(), "prisma", "recienscrapeados.ts");
...
function parseProxyConfig(proxyRaw: string) {
  if (!proxyRaw) return undefined;
  let p = proxyRaw.trim();
  if (!/^[a-zA-Z]+:\/\//.test(p)) p = "http://" + p;
  try {
    const u = new URL(p);
...
async function openContext(): Promise<{ browser: Browser; context: BrowserContext }> {
  ...
  const launchOpts: any = { headless: HEADLESS };
  if (proxyCfg) {
    launchOpts.proxy = {
  ...
  if (LOCALE) contextOpts.extraHTTPHeaders = { "accept-language": LOCALE };
...
async function getHandlesFromHashtag(page: any, hashtag: string, maxHandles = MAX_HANDLES_PER_TAG) {
  ...
  try {
    ...
    for (let i = 0; i < Math.min(videoIds.length, MAX_VIDEOS_PER_PROFILE); i++) {
      ...
      try {
        ...
        const isPinnedOnPage =
          /data-e2e=["']video-card-badge["'][^>]*>[\s\S]*?Pinned/i.test(vhtml) ||
