...
router.get("/", async (req, res) => {
    try {
        const { q, niches, country, minFollowers, maxFollowers, minEngagement, minAvgViews, minPrice, maxPrice, verified, hasContact, sortBy = "followers", order = "desc", page = "1", perPage = "25", } = req.query;
        ...
        let orderBy = { followers: "desc" }; // default
        ...
        const total = await prisma_1.prisma.tiktoker.count({ where });
        // fetch results
        const items = await prisma_1.prisma.tiktoker.findMany({
            where,
            skip,
            take: perPageNum,
            orderBy,
...
router.get("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const item = await prisma_1.prisma.tiktoker.findUnique({
            where: { id },
        });
        if (!item)
